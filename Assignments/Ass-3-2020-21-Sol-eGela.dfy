...
//EJERCICIO 1(b)
//DEMOSTRAR EL SIGUIENTE LEMMA POR INDUCCI�N EN xs.tail
...
{ //Por contradicci�n
    if !isSubList(xs.tail,ys) {
        ...
        if xs.head != ys.head {
		    ...
		    if xs.tail.Cons? { 
				tail_SubList_Lemma(xs.tail, ys.tail); 
...
{ //Por contradicci�n
	if vs.Cons? && !isSubList(us,vs) {
		...
		if us.head == vs.head { 
			if !isSubList(us.tail,vs.tail) { //Otra contradicci�n
	...
	ensures isSubList(filterTree(p,t),aplanar(t))
{
	...
	case Node(ti,r,td) => {
		...
		if p(r) {
			isSubList_append_Lemma(filterTree(p,ti),aplanar(ti),Cons(r,filterTree(p,td)),Cons(r,aplanar(td)));
			//Abajo
			//assert isSubList(append(filterTree(p,ti),Cons(r,filterTree(p,td))), 
			//                  append(aplanar(ti),Cons(r,aplanar(td))));
		}
		else { 
			cons_append_Lemma(aplanar(td));
	...
	ensures isSubList(append(xs,ys),append(vs,us))
{
	if xs == Nil {
		...
		if xs.head == vs.head { 
			isSubList_append_Lemma(xs.tail,vs.tail,ys,us); 
		}
		else { 
			isSubList_append_Lemma(xs,vs.tail,ys,us);
		}
	}
}
...
//Dise�ar un m�todo verificado que satisfaga el siguiente contrato:
...
//EJERCICIO 4
//Usando el m�todo de la bandera holandesa, 
//dise�ar un m�todo que tome como argumento un array de ENTEROS y una funci�n p: nat -> bool (o predicado),
//y clasifique los elementos del array en tres clases: los enteros negativos, los naturales que cumplen p
//y los naturales que no cumplen p. El orden relativo entre las tres clases y la posici�n relativa de 
