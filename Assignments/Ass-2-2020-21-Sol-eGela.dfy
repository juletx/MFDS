...
requires s1 < s2 && |s2| <= n // s1 está contenido en s2 y s2 tiene a lo sumo n elementos
ensures |s1 * s2| < n  // la intersección de s1 y s2 tiene menos de n elementos
{//prueba directa
assert s1 * s2 == s1;
...
requires s1 < s2 && |s2| <= n // s1 está contenido en s2 y s2 tiene a lo sumo n elementos
ensures |s1 * s2| < n  // la intersección de s1 y s2 tiene menos de n elementos
{//prueba por contradicción/ contraposición
if |s1 * s2| >= n
	{
	assert |s1 * s2| >= |s2|;
	assert s1 * s2 == s1;
