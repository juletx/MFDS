...
{
	if !mapInjective(m) {
		//var m' := map k | k in m :: m[k]; //ESTO ES LO MISMO QUE: 
		var m' := m;
		//assert exists a,b :: a in m && b in m ==> a != b ==> m[a] == m[b]; //ESTO ES LO QUE ESTÁ MAL NEGADO
		assert exists a,b :: a in m && b in m && a != b && m[a] == m[b];
		//var a, b :| a in m && b in m ==> a != b ==> m[a] == m[b]; //ESTO SE PUEDE HACER MEJOR
		var a, b :| a in m && b in m && a != b && m[a] == m[b];
		//sets_card_Lemma(m.Keys, m'.Keys);
		//assert |m.Keys| <= |m'.Keys|; //SON IGUALES, NO VALE PARA NADA
		//assert |m| == 2; //NO ES VERDAD
		//MIRA LO QUE TE FALTABA:
	    assert {a,b} <= m.Keys;
	    sets_card_Lemma({a,b}, m.Keys);
	    //assert |m| >= 2;
	}
}


// EJERCICIO 5 //PERFECTO
