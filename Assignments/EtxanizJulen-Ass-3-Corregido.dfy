...
/*{ //HABÍA QUE HACER CASOS DEPENDIENDO DE LA IGUALDAD ENTRE LAS CABEZAS DE LAS LISTAS
	match xs
		case Nil =>
		case Cons(h, t) => {
...
//    INDICACIÓN: UTILIZAR LE LEMMA tail_SubList_Lemma.  //MUY BIEN
...
/*{ //HABÍA QUE HACER CASOS DEPENDIENDO DE p(m) O !p(m)
	match t 
		case Leaf(x) =>
		case Node(l, m, r) => {
			if l.Node? {
...
/*{
	...
	{  //TUS ELEMENTOS SIN EXAMINAR ESTAN EN a[y..z), POR TANTO TIENES QUE MIRAR a[y], N0 a[x]
	   //SI CAMBIAS ESTE CÓDIGO SE VERIFICARÁ:
		if a[x] < 0 {
			a[x], a[y] := a[y], a[x];  
			x, y := x + 1, y + 1;
		}
